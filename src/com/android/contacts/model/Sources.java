...
public class Sources extends BroadcastReceiver implements OnAccountsUpdateListener {
    ...
    public void onReceive(Context context, Intent intent) {
        ...
        if (Intent.ACTION_PACKAGE_REMOVED.equals(action)
            ...
            //}
            if (pkgList != null) {
                for (String packageNameItem : pkgList) {
                    final boolean knownPackage = mKnownPackages.contains(packageNameItem);
                    if (knownPackage) {
                        // Invalidate cache of existing source
                        invalidateCache(packageNameItem);
                    } else {
    ...
    protected synchronized void queryAccounts() {
        ...
        // final IContentService cs = ContentResolver.ggetContentService();
        ...
        try {
            ...
            for (SyncAdapterType sync : syncs) {
                ...
                /*if (GoogleSource.ACCOUNT_TYPE.equals(accountType)) {
                    ...
                    source = new FallbackSource();//auth.packageName);
